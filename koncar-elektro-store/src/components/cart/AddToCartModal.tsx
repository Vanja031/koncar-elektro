...
export const AddToCartModal = () => {
  ...
  const product = addedSnapshot ? productById.get(addedSnapshot.productId) : undefined;
  const open = Boolean(addedSnapshot && product);

  useEffect(() => {
    if (!open || !addedSnapshot) return;
    const timer = window.setTimeout(closeAddedModal, AUTO_CLOSE_MS);
    return () => window.clearTimeout(timer);
  }, [addedSnapshot?.addedAt, closeAddedModal, open, addedSnapshot]);
    ...
    <Dialog open={open} onOpenChange={(next) => !next && closeAddedModal()}>
      <DialogPortal>
        ...
        {product && addedSnapshot ? (
          ...
          >
            <DialogPrimitive.Title className="sr-only">Proizvod dodat u korpu</DialogPrimitive.Title>
